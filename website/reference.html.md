...
| JSON Lines object streams | In a `.yaml` or `.yml` file, two or more JSON objects, one per physical line, become a multi-document YAML stream. Yamark inserts the `---` document-start marker before each record after the first; normal `--line-width` rules still control flow or block layout. |
...
| `--wrap sentence` | Put each Markdown sentence on its own line without column wrapping. |
| `--wrap sentence:<n>` | Put each Markdown sentence on its own line and wrap long sentences to column `<n>`. |
| `--wrap <n>` | Wrap Markdown prose to column `<n>` without forcing sentence boundaries. This is the default at column 72. |
...
Git-filter clean remains sentence-only and smudge remains column-only; neither
uses the combined default.
...
| `fmt: markdown wrap=sentence:<n>` | Mark the next YAML scalar as Markdown, put each sentence on its own line, and wrap long sentences to column `<n>`. |
...
`wrap` accepts `none`, `paragraph`, `sentence`, `sentence:<n>`, or a positive
integer. `sentence:<n>` forces sentence boundaries and wraps long sentences to
column `<n>`; a bare integer performs column wrapping without forcing sentence
boundaries.
